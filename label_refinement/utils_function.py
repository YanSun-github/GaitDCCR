...
def compute_cluster_property(labels, pseudo_labels):
    centers = collections.defaultdict(list)  # 当访问字典中不存在的键时会产生一个空列表作为默认值
    for i, pseudo_label in enumerate(pseudo_labels):
        if pseudo_label == -1:
            continue
        centers[pseudo_labels[i]].append(labels[i])

    tp = [
        max(collections.Counter(centers[idx]).values())/ len(centers[idx]) for idx in sorted(centers.keys())
    ]

    fp = [
        len(centers[idx]) - max(collections.Counter(centers[idx]).values())  for idx in sorted(centers.keys())
    ]

    fn = [
        110 - max(collections.Counter(centers[idx]).values()) for idx in sorted(centers.keys())
    ]

    pre = [tp[i] / (tp[i] + fp[i]) for i in range(len(centers))]
    rec = [tp[i] / (tp[i] + fn[i]) for i in range(len(centers))]
    f1_score = [ 2 * pre[i] * rec[i]/ (pre[i] + rec[i]) for i in range(len(centers))]
    return sum(f1_score)/len(centers)


def compute_true_centers(labels, pseudo_labels, features):
    centers = collections.defaultdict(list)  # 当访问字典中不存在的键时会产生一个空列表作为默认值
    centers_label = collections.defaultdict(list)
    for i, pseudo_label in enumerate(pseudo_labels):
        if pseudo_label == -1:
            continue
        centers_label[pseudo_labels[i]].append(labels[i])
    for i, (pseudo_label, label) in enumerate(zip(pseudo_labels, labels)):
        if pseudo_label == -1:
            continue
        if(label!=collections.Counter(centers_label[pseudo_label]).most_common(1)[0][0]):
            continue
...
def cluster_and_memory(model, epoch, args, use_leg=False):
    with torch.no_grad():
        ...
        part_features = compute_part_feature(part_features, part_num=1)
