...
class ClusterContrastTrainer(object):
    ...
    def train(self, args, epoch, data_loader, optimizer, pseudo_labeled_dataset, refinement_pseudo_labeled_dataset
        ...
        for i in range(train_iters):
            ...
            part_out = compute_part_feature(part_outs, part_num=1)
            with torch.no_grad():
            ...
            del refinement_labels, refinement_labels_p

            # p_s = extract_probabilities(f_out, self.memory.features)
            # p_t = extract_probabilities(ema_out, self.memory.features)
            # ce_loss =  torch.norm(p_s - p_t, p=2, dim=1).mean()
    ...
    def _inputs_pretreament(self, inputs, pseudo_labeled_dataset, refinement_pseudo_labeled_dataset, part_score):
        ...
        fnames = inputs[4]
        return ipts, ema_ipts, labels.cuda(), fnames, l, part_ipts, score
